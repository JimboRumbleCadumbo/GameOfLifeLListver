...
//-------------------------------------------NODE GETTERS--------------------------------------
    ...
    std::string getType();
    std::string switchNeighbor(int neighborType);
...
//-------------------------------------------NODE SETTERS--------------------------------------
    ...
    //Constructor
    explicit Node(bool alive = false,
         Node* top = nullptr,
         Node* topR = nullptr,
         Node* right = nullptr,
      ...
      this->isAlive = alive;
      this->futureLiving = alive;
      this->daysAlive = 0;
      }

    explicit Node(const std::vector<Node*> &neighbors = std::vector<Node*>(), bool alive = false) { 
